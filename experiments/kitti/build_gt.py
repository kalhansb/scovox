...
def main():
    ...
    # (notes: gt-accumulator-alternatives)

    # (notes: gt-boyer-moore-rejected)

    # (notes: gt-external-sort-option)

    # (notes: gt-sort-groupby-option)

    # Pass 1 writes each scan's unique (key, label) pairs, with a per-scan
    # majority label, to a temp file; pass 2 loads them, sorts by key and takes
    # each voxel's majority label. (notes: gt-two-pass-pairs)
