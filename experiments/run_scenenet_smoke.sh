#!/bin/bash
# SceneNet 0_223 smoke cell (Phase 0 Cell C), repointed to the scovox_ws layout.
# GT closed-set labels as input — the paper's SceneNet protocol.
set -o pipefail

WS=/home/kalhan/Projects/scovox_ws
SCOVOX_INSTALL="${WS}/scovox/install/setup.bash"
EVAL_PKG="${WS}/experiments/scovox_eval"
SCENENET_ROOT="${WS}/data/scenenet_val_layout"
RES_ROOT="${WS}/experiments/results/phase0_smoke/scenenet_0_223"
mkdir -p "${RES_ROOT}"
LOG="${RES_ROOT}/scovox.log"; OUT="${RES_ROOT}/scovox.npz"

export PATH=$(echo "$PATH" | tr ':' '\n' | grep -v miniconda | tr '\n' ':' | sed 's/:$//')
source /opt/ros/humble/setup.bash
source "${SCOVOX_INSTALL}"
export PYTHONPATH="${EVAL_PKG}:${PYTHONPATH:-}"

echo "== launch scovox_mapping_node (SceneNet eval) =="
...
echo "== replay SceneNet 0_223 (GT labels) =="
