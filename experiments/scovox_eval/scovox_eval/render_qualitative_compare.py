...
def render_kitti(ws_root: Path, seq: str, out_dir: Path,
                 ...
                 # cam0 frame: X right, Y down (gravity), Z forward. y_clip
                 # keeps about 5 m above to 2 m below road level;
                 # horizontal_half_extent is half the side of the square
                 # horizontal clip. (notes: render-kitti-clip-window)
    ...
    # SCovox and GT use the cam0 world frame (P @ Tr); SLIM-VDB uses the
    # velodyne world frame (Tr_inv @ P @ Tr). Left-multiplying by Tr (velo to
    # cam0) brings SLIM-VDB into the GT frame.
    # (notes: render-kitti-slimvdb-frame)
    Tr = np.eye(4, dtype=np.float64)
