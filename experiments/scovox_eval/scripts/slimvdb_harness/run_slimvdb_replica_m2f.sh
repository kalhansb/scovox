...
# Mask2Former ADE20K predictions from data/replica_niceslam/<scene>/semantic_m2f_ade/.
# Paired build: NCLASSES=151 (150 ADE20K classes + 0 void).
...
OUT_ROOT=${WS}/third_party_sw/slim_vdb/outputs/replica_m2f
mkdir -p "${OUT_ROOT}"
...
for scene in "${SCENES[@]}"; do
    ...
    echo ""
    echo "============================================================"
    echo " SLIM-VDB M2F on Replica scene ${scene}  (n_scans=${N_SCANS})"
    echo "============================================================"
    ...
    docker run --rm --gpus all \
        ...
        "${IMG}" bash -c "cp ${BIN} . && ./replica_pipeline /workspace/data/replica_niceslam /workspace/third_party_sw/slim_vdb/outputs/replica_m2f/${scene} --sequence ${scene} --config ${CFG} --n_scans ${N_SCANS}" \
        2>&1 | tee "${TIMING_LOG}"
