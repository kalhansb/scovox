...
# NCLASSES=151 build matches Mask2Former ADE20K (150 classes + 0 void).
...
for scene in "${SCENES[@]}"; do
    ...
    echo ""
    echo "============================================================"
    echo " SLIM-VDB on Replica scene ${scene}  (n_scans=${N_SCANS})"
    echo "============================================================"
    ...
    docker run --rm --gpus all \
        ...
        "${IMG}" bash -c "cp ${BIN} . && ./replica_pipeline /workspace/data/replica_niceslam /workspace/third_party_sw/slim_vdb/outputs/replica/${scene} --sequence ${scene} --config config/replica.yaml --n_scans ${N_SCANS}" \
        2>&1 | tee "${TIMING_LOG}"
