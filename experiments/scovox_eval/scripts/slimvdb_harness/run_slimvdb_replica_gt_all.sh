#!/bin/bash
# GT-oracle SLIM-VDB runs on all 8 Replica scenes — uses `semantic_gt_fixed/`
# PNGs (Replica 101-class space, re-rendered via Habitat-sim by the parallel
# session). Runs the NCLASSES=102 binary (build-replica/bin/replica_pipeline)
# and writes outputs to outputs/replica_gt/<scene>/.
set -euo pipefail
...
for scene in "${SCENES[@]}"; do
    ...
    echo ""
    echo "============================================================"
    echo " SLIM-VDB GT oracle on Replica scene ${scene}  (n_scans=${N_SCANS})"
    echo "============================================================"
    ...
    docker run --rm --gpus all \
        ...
        "${IMG}" bash -c "cp ${BIN} . && ./replica_pipeline /workspace/data/replica_niceslam /workspace/third_party_sw/slim_vdb/outputs/replica_gt/${scene} --sequence ${scene} --config config/replica_gt.yaml --n_scans ${N_SCANS}" \
        2>&1 | tee "${TIMING_LOG}"
