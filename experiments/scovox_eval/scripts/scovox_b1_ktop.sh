#!/bin/bash
# B1 ablation: K_TOP sparse Dirichlet capacity.
# Recompile-driven sweep — set K_TOP in scovox_core/include/scovox/voxel.hpp,
# rebuild Release, then run this script to capture the baseline cell of both
# anchors into a dedicated k_top_<K> cell.
#
# Usage:  scovox_b1_ktop.sh <K>          # both anchors
#         scovox_b1_ktop.sh <K> replica  # replica room0 m2f only
#         scovox_b1_ktop.sh <K> kitti    # kitti seq08 polarseg only
#
# Each cell: results/ablations_<anchor>/k_top_<K>/scovox.npz + scovox_run.log
set -eo pipefail
...
if [[ "${SRC_K}" != "${K}" ]]; then
  ...
  echo "       edit ${VOXEL_HPP} and rebuild before running." >&2
  exit 3
fi
...
run_replica() {
  local CELL_DIR="${EVAL_PKG}/results/ablations_replica_room0_m2f/k_top_${K}"
  local OUT="${CELL_DIR}/scovox.npz"
  local LOG="${CELL_DIR}/scovox_run.log"
  mkdir -p "${CELL_DIR}"
  if [[ -f "${OUT}" ]]; then echo "[replica k_top_${K}] skip"; return; fi
  echo "━━ replica room0 m2f, K_TOP=${K} ━━"
...
run_kitti() {
  local CELL_DIR="${EVAL_PKG}/results/ablations_kitti_seq08_polarseg/k_top_${K}"
  local OUT="${CELL_DIR}/scovox.npz"
  local LOG="${CELL_DIR}/scovox_run.log"
  mkdir -p "${CELL_DIR}"
  if [[ -f "${OUT}" ]]; then echo "[kitti k_top_${K}] skip"; return; fi
  echo "━━ kitti seq08 polarseg, K_TOP=${K} ━━"
  ...
  python3 -m scovox_eval.semantickitti_replay_node --ros-args \
      -p dataset_path:="${WS}/data/semantickitti/dataset" \
      -p sequence:=8 \
      -p rate_hz:=0.5 \
      -p robot_name:=ablation \
      -p max_range:=30.0 \
