#!/bin/bash
# SceneNet SOFT-label run with a chosen semantic_mode, for the ablation.
# Usage: run_scenenet_soft_mode.sh <seq> <semantic_mode> <out_root>
set -o pipefail
SEQ="${1:-0_223}"
SEM_MODE="${2:-dirichlet}"
OUT_ROOT="${3:-/home/kalhan/Projects/scovox_ws/experiments/results/scenenet_soft}"
...
echo "== launch scovox (SceneNet SOFT, mode=${SEM_MODE}, seq=${SEQ}) =="
ros2 launch scovox_mapping scenenet_eval.launch.py \
    robot_name:=phase0 resolution:=0.05 semantic_mode:="${SEM_MODE}" \
