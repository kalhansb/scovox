#!/bin/bash
# Post-revert smoke test: re-run Replica room0 with reach_prob removed +
# carve_skip_occ_threshold=0.7 restored. Compares against b7_moderate_recheck
# (post-cleanup-with-reach_prob) and the pre-cleanup baseline.
set -eo pipefail
...
CELL_NAME="post_revert_default"
CELL_DIR="${RESULTS_ROOT}/${CELL_NAME}"
OUT="${CELL_DIR}/scovox.npz"
LOG="${CELL_DIR}/scovox_run.log"
mkdir -p "${CELL_DIR}"

if [[ -f "${OUT}" ]]; then echo "[skip] ${CELL_NAME}"; exit 0; fi

echo "━━ ${CELL_NAME}  (w_occ=2, w_free=1, gate=0.5, csk=0.7, no reach_prob) ━━"
