...
def _launch_setup(context):
    ...
    scovox_node = Node(
        ...
        parameters=[{
            ...
            # Map model. leaf_bits sets the sparse grid's block size. It moves
            # no bits of map state — every value stores the same map — so it is
            # a pure speed/memory axis. A small leaf was once chosen here on
            # the theory that sparse outdoor LiDAR wastes less memory in a
            # 2×2×2 block; per-block overhead dominates instead, which makes
            # the small leaf both slower and larger.
