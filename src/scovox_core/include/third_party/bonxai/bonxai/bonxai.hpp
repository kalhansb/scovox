...
class VoxelGrid {
 ...
 public:
  ...
  /// Detailed memory breakdown for diagnostics and paper tables.
  struct MemDetail {
    size_t inner_count;        ///< number of inner grid nodes
    size_t leaf_count;         ///< number of active leaf blocks
    size_t active_voxels;      ///< total active voxels across all leaves
    size_t voxels_per_leaf;    ///< capacity per leaf (dim^3)
    size_t root_map_bytes;     ///< hash-map bucket + entry overhead
    size_t inner_grid_bytes;   ///< inner-node masks + shared_ptr arrays
    size_t leaf_meta_bytes;    ///< leaf masks + per-leaf metadata (excl. voxel data)
    size_t pool_alloc_bytes;   ///< pool allocator total allocated
    size_t pool_capacity;      ///< pool block capacity
    size_t pool_used;          ///< pool blocks in use
    size_t total_bytes;        ///< everything summed (should match memUsage())

    float  avg_active_per_leaf() const { return leaf_count > 0 ? float(active_voxels) / leaf_count : 0.f; }
    float  leaf_fill_ratio()     const { return (leaf_count > 0 && voxels_per_leaf > 0) ? float(active_voxels) / (leaf_count * voxels_per_leaf) : 0.f; }
    size_t pool_waste_bytes()    const { return pool_alloc_bytes > 0 ? pool_alloc_bytes - pool_used * voxels_per_leaf * sizeof(DataT) : 0; }
  };

  [[nodiscard]] MemDetail memUsageDetailed() const;
...
template <typename DataT>
inline typename VoxelGrid<DataT>::MemDetail VoxelGrid<DataT>::memUsageDetailed() const {
  MemDetail d{};
  d.voxels_per_leaf = 1u << (LEAF_BITS * 3);  // dim^3

  // Root map overhead
  for (unsigned i = 0; i < root_map.bucket_count(); ++i) {
    size_t bs = root_map.bucket_size(i);
    d.root_map_bytes += (bs == 0) ? 1 : bs;
  }
  d.root_map_bytes += root_map.size() * (sizeof(CoordT) + sizeof(void*));

  d.inner_count = root_map.size();

  // Inner grids + leaf grids
  for (const auto& [key, inner_grid] : root_map) {
    d.inner_grid_bytes += inner_grid.memUsage();
    for (auto inner_it = inner_grid.mask().beginOn(); inner_it; ++inner_it) {
      d.leaf_count++;
      auto& leaf_grid = inner_grid.cell(*inner_it);
      d.leaf_meta_bytes += leaf_grid->memUsage();  // mask + metadata only (pool = external)
      // Count active voxels in this leaf
  ...
  // Pool allocator
  d.pool_alloc_bytes = leaf_block_allocator_.memUsage();
  d.pool_capacity = leaf_block_allocator_.capacity();
  d.pool_used = leaf_block_allocator_.size();

  d.total_bytes = d.root_map_bytes + d.inner_grid_bytes + d.leaf_meta_bytes + d.pool_alloc_bytes;
  return d;
}
