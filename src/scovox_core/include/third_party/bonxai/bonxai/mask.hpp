...
class Mask {
  ...
  // small object optimization: inline a single word (64 bits, enough for
  // LOG2DIM <= 1, i.e. up to 2×2×2 = 8 voxels per leaf).
  // Anything larger heap-allocates. Previous value was [8] which wasted
  // 56 bytes per Mask when LOG2DIM == 1.
  uint64_t static_words_[1];
...
inline Mask::Mask(size_t log2dim)
  ...
  words_ = (WORD_COUNT <= 1) ? static_words_ : new uint64_t[WORD_COUNT];
...
inline Mask::Mask(Mask&& other)
  ...
  if (WORD_COUNT <= 1) {
    ...
    // Steal heap pointer; null out other so its destructor is safe.
    words_ = other.words_;
...
inline Mask::~Mask() {
  if (WORD_COUNT > 1 && words_ != nullptr) {
    delete[] words_;
  }
}
