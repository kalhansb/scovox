...
struct Params {
  ...
  uint8_t leaf_bits  = 3;  ///< Leaf block dimension bits (3 → 8×8×8 = 512 voxels/block)
                           ...
                           ///< to reduce waste" is RETIRED: 1 measured as the worst rung
                           ///< on sparse LiDAR -- slower AND larger, map bit-identical --
                           ///< so the LiDAR configs now carry this default rather than
                           ///< overriding it.  A block that is too small pays more
                           ///< allocations and more pointer chasing than the empty
                           ///< voxels of a bigger one cost.
