...
**Scope.** Read-only review of the working tree at commit `2deeeef` plus its
...
| H1 | High | `scovox_node` defaults are not the promoted configuration | **resolved** — `config/scovox_best_method.yaml`, commit `142545f` |
| H2 | High | The promoted state is uncommitted (31 files since `2deeeef`) | **resolved** — `d0a3c3e`, `142545f` |
...
does: `0f0c490` (hit batching default-on, `uint16` Beta), `2deeeef` (the exact
Amanatides & Woo walk replaces Bresenham unconditionally) and `d0a3c3e` (the
...
**Impact.** Commit `0f0c490` is titled "Put the promoted configuration into
...
**Resolved (`142545f`) — second option taken, deliberately.** Moving the twelve
...
**Resolved.** `d0a3c3e` ("Promote the count-storage design; take doc pointers
and results out of code") carries the total basis, the `nhit` removal and the
`SCOVOX_BETA_U16` default flip; `142545f` carries the config file and a test
...
`2deeeef` and the uncommitted total-basis work.
...
**Fix.** Bisect the working-tree diff against `2deeeef` on this one test;
...
24 remaining `.md` references in code were removed or repointed in `d0a3c3e`,
...
**Resolved** in `d0a3c3e`: the six paths were corrected and the index now links
...
`3ca5285` refreshed a batch of stale `file:line` pointers by applying a
...
| `integrateHitSplit` `:575-595` has "no `tsdf_enabled_` test" | `structure.md:449` | `:629-655`, and it *does* test it (`:648`) — `231f2ac` fixed that, and the doc kept asserting the bug after its own fix |
...
| "§2 is this repository at commit `142545f`" | `structure.md:10` | 27 commits behind; re-pinned to `7f45ec9` |
...
- **`e129ae3`: "make version.hpp the binary's self-report."** The print was added
  to `replay_scenenn` but `version.cpp` was never added to the offline harness's
  `scovox_core` source list, so `replay_scenenn` **did not link** from `e129ae3`
...
- **`838414a`: "69 sites renamed", "7 bare `0xFFFF` remain".** Re-counted at
  `7f45ec9`: 67 occurrences on 64 lines were renamed, and **11** bare `0xFFFF`
...
- **`aae9366`: "replaced three `file:line` pointers".** It replaced **five**
  (`voxel.hpp:39-42`, `scovox_node.cpp:860-863`, `dir_voxel.hpp:250`,
  `sem_split_map.cpp:130-168`, `dir_voxel.hpp:233-236`).
- **Four consecutive commits do not compile.** `838414a`, `3b3c8db`, `e816290`
  and `e50a2c3` each call `scovox::gateAndRefresh` / `emitSnapshotOrTouched`
  while `node_utils.hpp` still defines them behind a `def=0` guard; the helpers
  only arrive in `4138467`. Verified by `git show <commit>:<file>`. `git bisect`
  across this range will report "cannot build" on four points. Repairing it means
  splitting `4138467` and rebasing, which this series has deliberately not done:
  the commits are local and unpushed, a backup branch
  `backup/review-series-pre-split` exists at `f477f37`, and the rewrite is
  waiting on an explicit decision rather than being taken unilaterally.

### Rejected

- **`3b3c8db`: "three bare `5`s in two packages."** Checked against the diff:
...
- **Byte identity against HEAD itself.** `scripts/pristine_head_replay.sh`
  compiles a `git archive HEAD` snapshot of both trees — submodule `7f45ec9`,
