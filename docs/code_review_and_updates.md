...
(3 421 lines, `SCovoxNode`) at commit `c92dc31`, working tree clean for this
...
against the tree at `ccf1e23`, which differs from `c92dc31` by one docs
...
commits landed after most of it was written (`0f0c490` batching default-on,
`d0a3c3e` count storage + `quality` removed + `nhit` compiled out, `2deeeef`
...
`ccf1e23`. Its S-series was fixed by the eleven commits `838414a..7f45ec9`
...
line numbers: the edit described in G14 landed as `1401373` (2026-09-06
...
document is a `ccf1e23` number (3 421 lines); add 7 to anything past `:839`
...
`ccf1e23` unless another file is named (see the second addendum for the
...
| G14 | G | sparse-branch counters compiled out | landed (`1401373`); proof items (1), (2), (5) open |
...
the ROS node used them for range decay. Two days later `d0a3c3e` deleted the
...
| cleanup item | claim (as written) | status at `ccf1e23` | evidence |
|---|---|---|---|
| 1 `quality` | inert here, live range-decay channel in the node; do NOT delete | **overtaken: deleted by `d0a3c3e`** | `scovox_map_split.hpp:161-165` and `:747-759` take no `quality`; the node calls `integrateHit(O, Hp, rng, cp, prof)` at `:1272`, `:1779`, `:1823`; `grep 'std::exp(-'` on the node is empty |
...
| order 7–9 withdrawn | keep `range_decay_length`, keep `quality`, keep `cls` u16 | 7 and 8 were **reversed by `d0a3c3e`** for `quality`; 9 stands | |
...
- [ ] **F1 — Rename or restore `range_decay_length`; fix the header that
  ...
  no caller applies it since `d0a3c3e`. `split_memory_demo.cpp:97` sets it to
...
- [ ] **F3 — Put a status banner on `scovox_cleanup.md`.** Its 2026-09-02
  ...
  field") describe a tree that `d0a3c3e` changed two days later, and the
...
tier does not restate it; it re-reads every status against `ccf1e23`, records
...
`838414a..7f45ec9` on 2026-09-05 and every fix is still in the tree. The
...
the review was written. The edit that became `1401373` described in the
second addendum is the review's own closing recommendation being acted on
(G14); it is recorded here, not started here.

### Status of every item

| id | finding (as written) | status as written | status at `ccf1e23` | evidence |
|---|---|---|---|---|
| H1 | node defaults are not the promoted configuration | resolved (`142545f`) | resolved | `config/scovox_best_method.yaml` present; the other two RGB-D yamls cite it |
| H2 | the promoted state is uncommitted | resolved (`d0a3c3e`, `142545f`) | resolved | both commits in `git log` |
| H3 | `integrateHitSplit` ignores `tsdf_enabled` | resolved (`231f2ac`) | resolved | `scovox_map_split.hpp:289` and `:564` test `tsdf_enabled_` |
...
| errata | four commits do not compile | recorded, not fixed; decision pending | **still pending (G13)** | `838414a`, `3b3c8db`, `e816290`, `e50a2c3` precede `4138467` in `git log`; `new_experiments` is 39 commits ahead of `origin`; `backup/review-series-pre-split` exists |
...
- [ ] **G13 — Decide the history rewrite before the next push (errata).**
  `838414a`, `3b3c8db`, `e816290` and `e50a2c3` each call `gateAndRefresh` /
  `emitSnapshotOrTouched` before `4138467` defines them, so `git bisect`
  cannot build across four commits. The branch is still 39 commits ahead of
  `origin/new_experiments`, so the rewrite (split `4138467`, rebase) is still
  cheap, and `backup/review-series-pre-split` at `f477f37` is in place. Every
...
- [ ] **G14 — Compile out the sparse-branch counters: landed as `1401373`
  ...
  byte-identical candidate on the deposit path. `1401373` does it:
...
- [x] **G15 — `buildSwitches()` names seven of the nine live switches.**
  **Closed 2026-09-07.** All ten live switches are now on the line; see the
  ...
  S9 (`e129ae3`) made `version.cpp` the binary's self-report under the
  ...
  (`scovox_map_split.hpp:349-352`, from `c92dc31`) and
  `SCOVOX_SPARSE_BRANCH_COUNTERS` (`voxel.hpp:92-97`, from `1401373`).
...
3. **D:** record the `/scovox/bin` wire frames (and the `[tsdf_dump]` file
   if `tsdf_dump` is on) from one bag on `c92dc31` and on the branch, same
...
10. Own commits, when their decision is made: G9 (with its tests), G12
    ...
    as `1401373`; its open proof items need no further commit. G16 belongs
