...
- **§2 The current code** is this repository at commit `8b01ed3`, re-pinned
  and re-audited on 2026-09-06. It was first written against `142545f` and the
  ...
  block grew under it), and five commits since `7f45ec9` (`0326f6b`, `bef6af9`,
  `8ce3e80`, `c92dc31`, `1401373`) had moved `scovox_map_split.hpp` by up to
  96 lines and `sem_split_map.cpp` by about ten. Every pointer below was
  re-read against `8b01ed3`. Treat any pointer here as an address that
  ...
  `SCOVOX_BETA_U16` default flip were committed in `d0a3c3e`; the
  promoted-configuration file and an off-lattice test fix in `142545f`.
...
numbers are line numbers at commit `8b01ed3` (2026-09-06) and will
...
are working-tree addresses from that change, not `8b01ed3` ones. When a pointer here disagrees with the
...
`docs/archive/scovox_code_structure.md` and describes the pre-`2deeeef` tree.
...
> `0f0c490` (hit batching default-on, `uint16` Beta), `2deeeef` (the exact walk
> replaces the earlier approximate traversal) and `d0a3c3e` (count storage). Re-running the promoted arm
...
Line counts at `8b01ed3`: `scovox_node.cpp` 3437, `sem_split_map.cpp`
...
- **`integrateHitSplit`** (`:725-751`): two DDAs; calls `tsdf_.integrateRay`
  ...
  `231f2ac`, so a split-path run paid in full for a TSDF the flag declared
...
`feedback_slimvdb_memory_measurement.md` citation was removed in `d0a3c3e`;
...
| `ray_iterator.hpp` | exact DDA iterator (the only traversal since `2deeeef`) |
...
Commit `0f0c490` ("Put the promoted configuration into the code") changed the
...
**Addressed in `142545f`, without moving these defaults.** They are shared with
...
- The LiDAR configs (`w_occ` 8, `w_free` 4, res 0.10) are a different
  ...
  as the best method.
- `scovox_best_method.yaml` (added `142545f`) is the promoted RGB-D
...
- `integrateHitSplit` ran the TSDF DDA regardless of `tsdf_enabled` until
  `231f2ac` (2026-09-04); the gate now sits at `scovox_map_split.hpp:744`.
...
The `.md` citations are a separate matter. `d0a3c3e` swept `src` and
...
method; only the ROS surface around it is behind — and since `142545f` that gap
